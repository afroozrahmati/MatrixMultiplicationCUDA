 ...
 */
 ...
 /* 
 functionality: 2D naive Matrix Multiplication Implementation a*b=c
 input parameters: 
             a         : the input matrix
             b         : the input matrix
             c         : the result matrix
 ...
 __global__ void matMulGPU(float* A, float* B, float* C, int n) {
    ...
    // if (idx < TILE_WIDTH * TILE_WIDTH)
    // {
    //     float Cvalue=0.0;

    //     for (int i = 0; i< TILE_WIDTH; i++)
    //     {
    //         Cvalue += A[ y* TILE_WIDTH + i] * B[i * TILE_WIDTH + x];
    //     }
        
    //     C[ y * TILE_WIDTH + x] = Cvalue;
    // }
 ...
 */
 ...
 ///////////////////////**************************************************
     ...
     //dim3 blockPerGrid(2,2,1);      
     //dim3 threadPerBlock(2,2,1); 

    // dim3 blockPerGrid(ceil(N/(float)TILE_WIDTH), ceil(N/(float)TILE_WIDTH), 1);
    // dim3 threadPerBlock(TILE_WIDTH, TILE_WIDTH, 1);
     ...
     matMulGPU<<<dimGrid, dimBlock>>>( d_a, d_b, d_c_out_naive, N);
